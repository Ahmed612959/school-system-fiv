...
// ====================== إعداد رفع الملفات للذكاء الاصطناعي ======================
...
// استيراد دوال الذكاء الاصطناعي
const { trainOnFile } = require('./ai-train');
const { askQuestion, loadAllChunks } = require('./ai-chat');
...
app.post('/api/ai/train', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ error: 'يرجى اختيار ملف أولاً' });
        }
        
        const filePath = req.file.path;
        const fileName = req.file.originalname;
        
        const result = await trainOnFile(filePath, fileName);
        
        if (result.success) {
            res.json({ 
                success: true, 
                message: `تم تدريب الذكاء الاصطناعي على الملف: ${fileName}`,
...
// جلب معلومات عن حالة الذكاء الاصطناعي
app.get('/api/ai/status', async (req, res) => {
    try {
        const allChunks = loadAllChunks();
        const fileCount = allChunks.length > 0 ? new Set(allChunks.map(c => c.source)).size : 0;
        
        res.json({
            trained: allChunks.length > 0,
            chunksCount: allChunks.length,
            fileCount: fileCount,
            lastUpdate: allChunks.length > 0 ? allChunks[allChunks.length - 1]?.timestamp : null
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
