...
// أضف هذا القسم في api/index.js (قبل module.exports)
...
const { trainOnFile, clearAllData } = require('./ai-train');
const { askQuestion } = require('./ai-chat');

// رفع ملف للتدريب
app.post('/api/ai/train', aiUpload.single('file'), async (req, res) => {
    try {
...
app.get('/api/ai/status', async (req, res) => {
    try {
        const { loadAllChunks } = require('./ai-chat');
        const allChunks = loadAllChunks();
        const fileCount = new Set(allChunks.map(c => c.source)).size;
        res.json({ trained: allChunks.length > 0, chunksCount: allChunks.length, fileCount: fileCount });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
